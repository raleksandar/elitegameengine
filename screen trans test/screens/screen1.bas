...
Namespace Screen1
	...
	' called when first screen should be shown
	Sub Game_onReady
	...
	Sub Screen_onShow
		...
		background.ApplyFilter Engine.fbWater
		
		nextScreen.Create Color.Green
		...
		prevScreen.Create Color.Green, Elite.True
	...
	Sub Screen_onFrame(fElapsed As Single, eInputEvent As Engine.InputEvent, eventData As FB.EVENT)
		...
		If eInputEvent = Engine.KeyPress Then					' key was pressed
			...
			Select Case As Const eventData.scanCode
				...
				Case Engine.KeySpace:		ShowScreen Screen2.GameScreen
					
				Case Engine.KeyBackSpace:	ShowScreen Screen4.GameScreen
		...
		ElseIf eInputEvent = Engine.MouseButtonRelease Then		' mouse button was released
			...
			If prevScreen.Pressed Then
				...
				If prevScreen.Hovered Then ShowScreen Screen4.GameScreen
			
			ElseIf nextScreen.Pressed Then
				
				nextScreen.Pressed = False
				
				If nextScreen.Hovered Then ShowScreen Screen2.GameScreen
